...
mpg_make_model <- mpg %>% # Start with mpg
  unite(col = make_model, # Create a new column called 'make_model'
        ...
        sep = "YoLo!" # Puts "HEY!" between the pieces (default is underscore)
        ) 
...
mpg_model_class <- mpg %>% # Start with mpg
  unite(col = model_class, # Create a new column called 'mpg_model_class'
        c(model, class),  # That combines the model and class columns
        sep = "-", # Puts a dash between the 2 pieces
        remove = FALSE) # And do *not* remove the original column
```
...
>>>>>>> 9d53dca7447eb4c9f05287d7d94a7fc031fb3608
ymd("1998/07/01")
...
lakers_dates <- lakers %>% # Start with lakers data
  mutate(date_new = ymd(date)) %>% # Update the date column to ymd format
  mutate(year_new = year(date_new)) %>% # Add a new year column
  mutate(month_new = month(date_new)) # Add a new month column
...
points_by_month <- lakers_dates %>% # Start with lakers_dates data
  group_by(game_type, month_new) %>%  # Group by game type and month_new
  summarize(total_points = sum(points)) # Summarize total points per game type per month
...
gasol_hookshots <- lakers %>% # Start with lakers data
  filter(player == "Pau Gasol", type == "hook") %>% # Filter player to Pau Gasol and shot type to hook
  mutate(date_new = ymd(date)) %>% # Update the date column to ymd format
  mutate(month_new = month(date_new)) # Add new month column
  
# Now run this code to calculate Gasol's hook shots by month for each *month* in the data:
...
storms_danger <- storms %>% # Start from storms data
  mutate(danger = case_when( # Add a new column, danger, which contains... 
    ...
    TRUE ~ "moderate")) # otherwise, "moderate" if status is anything else
```
